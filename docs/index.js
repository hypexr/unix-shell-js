(function (exports) {
    ...
    class UnixShell {
                        ...
                        if (isDir) {
                            ...
                            if (humanReadable) {
                                if (bytes < 1024)
                                    ...
                                    size = Math.round(bytes / 1024) + 'K';
                                else
                                    size = Math.round(bytes / (1024 * 1024)) + 'M';
                            }
                            else {
                                size = bytes.toString();
                            }
        ...
        writeToFile(filePath, content, mode) {
            ...
            if (mode === 'append' && fileName in parent) {
                parent[fileName] = parent[fileName] + content;
            }
            else {
        ...
        openEditor(filename) {
            ...
            const exitCallback = () => {
                if (typeof window !== 'undefined' && window.enableTerminal) {
                    window.enableTerminal();
                }
            };
            if (typeof window !== 'undefined' && window.disableTerminal) {
    ...
    exports.UnixShell = UnixShell;
    exports.default = UnixShell;

    Object.defineProperty(exports, '__esModule', { value: true });

})(this.UnixShellModule = this.UnixShellModule || {});
